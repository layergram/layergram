...
**Note on transport compatibility:** Steganographic embedding uses invisible Unicode characters and requires a transport that preserves them. For transports that strip or normalize zero-width characters, Layergram supports alternative **direct text payload** (`<payload>`) and **direct message link** (`layergram://m/<payload>`) formats. Direct text payloads avoid the Layergram URI scheme but still expose an opaque ciphertext-sized blob; direct links make the presence of encrypted communication more visually obvious to anyone who sees the link.
...
- **Future compromise of long-term keys (partial forward secrecy).** Base (legacy) message encryption uses static–static X25519 between identity keys; if a long-term identity key is later compromised, past messages encrypted with it for the corresponding peer can be decrypted. Layergram now also supports **opportunistic Forward Secrecy**: once a compatible session is negotiated and confirmed, messages are protected by an ephemeral Double Ratchet whose past keys cannot be reconstructed from the long-term identity key. Until a session is confirmed — and for contacts/devices that never negotiate FS — Layergram falls back to the legacy model for compatibility, so protection of past messages after a future key compromise is only guaranteed for messages sent under a confirmed FS session. See the "Forward Secrecy" section below and [specs/FORWARD_SECRECY.md](specs/FORWARD_SECRECY.md).
...
**Passphrase-Derived Identities.** A passphrase may derive an alternative Layergram identity from the same recovery phrase. Layergram does not persist passphrase-derived identities as local profile entries. When a passphrase is not active, the derived identity key material is not present in memory and any related local records remain opaque encrypted data.

**Local Auxiliary State.** Forward Secrecy state, passphrase-specific security settings, and related protocol state may be stored as encrypted auxiliary records inside the local encrypted archive area. These records are padded and externally shaped like other encrypted archive records. Their semantic type is only visible after successful decryption in the correct identity context; new records do not expose cleartext aux markers.

**Limitations.** Layergram does not protect against a compromised device, malware, keyloggers, screenshots, clipboard leakage, operating-system backups, or seizure while an identity is actively unlocked. Layergram protects message content, not metadata exposed to the external transport application. Forward Secrecy reduces the value of a future long-term key compromise but does not protect plaintext that is decrypted and cached while an identity is unlocked, nor does it claim perfect zeroization of secrets in a managed runtime (see [specs/FORWARD_SECRECY.md](specs/FORWARD_SECRECY.md) → "Secure memory handling limitations").
