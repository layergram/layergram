...
**Maximum Forward Secrecy.** Maximum FS does not make the external transport reliable: an active attacker can still prevent the required `fs_init`, `fs_reply`, or `fs_confirm` messages from being delivered. Its security benefit is that this failure is not silent. A requested Maximum FS contact remains pending until the strict session is confirmed. While pending or under repair, Layergram may send FS setup/control messages, but not user-authored plaintext; the user's actual message remains unsent until Maximum FS is active. Once Strict/Maximum FS is active, Layergram must not silently fall back to legacy encryption. If the active strict session is replaced by a new pending session or an unexpected device appears, sending is paused until the user repairs the session or disables Maximum FS.
